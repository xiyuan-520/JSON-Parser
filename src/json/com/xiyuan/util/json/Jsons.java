...
public final class Jsons implements Serializable
{
     ...
     */
    @SuppressWarnings("unchecked")
    public static <T, K, V> Map<K, V> toMap(String json, Class<T> mapClass, Class<K> keyClass, Class<V> valueClass)
    {
        boolean isMap = false;
        JsonLexer lexer = new JsonLexer(json);
        while (lexer.hasNext())
        ...
        if (!isMap)
            return ((MapParser) lexer.MapParser()).newMap(mapClass, keyClass, valueClass, 0);
