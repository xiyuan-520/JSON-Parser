        ...
        while (lexer.hasNext())
        {
            ...
            if (key != null)
            {
                ...
                if (parser == lexer.ListParser())
                    value = ((ListParser) lexer.ListParser()).toObject(valueType, JsonLexer.getClass(key.getGenericType(), 0));
                else if (parser == lexer.MapParser())
                    value = ((MapParser) lexer.MapParser()).toObject(valueType, JsonLexer.getClass(key.getGenericType(), 0), JsonLexer.getClass(key.getGenericType(), 1));
                else
                    value = parser.toObject(valueType);
